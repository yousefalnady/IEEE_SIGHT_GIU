...
GPIO.setup(6, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)  # Button
...
def ultrasonic_monitoring():
    while True:
        with lock:
            # Add ultrasonic sensor monitoring code here
...
def button_handler():
    ...
    while True:
        if GPIO.input(6) == GPIO.HIGH:
            ...
            with lock:
               with open("nav_test.py") as file:
                   exec(file.read())  
            print('navigating home 5alsana')
...
# Schedule read_gps_data() function to run every 2 minutes
def run_read_gps_data():
    read_gps_data()
    threading.Timer(40, run_read_gps_data).start()
